...
class CorefModel(object):
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    # beam size
    k = tf.minimum(300, tf.to_int32(tf.floor(tf.to_float(num_words) * self.config["top_span_ratio"])))
  ...
  def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends):
    ...
    if self.config["use_features"]:
      ...
      span_width_embeddings = tf.get_variable("span_width_embeddings", [self.config["max_span_width"], self.config["feature_size"]], initializer=tf.truncated_normal_initializer(stddev=0.02))
